...
function renderStages(){const names=["Intake","Extract","Classify","Policy checks","Internal gate","Response gate"];return `<section class="stage-strip" aria-label="Workflow stages">${names.map((name,index)=>`<div class="stage ${stageState(index)}"><div class="stage-top"><span class="stage-index">0${index+1}</span><span class="stage-status" aria-hidden="true"></span></div><strong>${name}</strong></div>`).join("")}</section>`}
